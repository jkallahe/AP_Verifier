...
public class Ssc
{
	...
	private static final int MAX_CHARS = 8; //greatest length of the short string
	...
	public String get_comparison_string(String hash_val)
	{
		...
		output_str = short_str.toString(); //create a string representation of the buffer
		for(i = output_str.length(); i < MAX_CHARS; i++) //zero-pad to full length (if using a shorter hash algorithm)
		{
			if(i < MAX_CHARS)
			{
				output_str += "0";
